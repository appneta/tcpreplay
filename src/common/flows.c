        ...
        const u_char *pktdata, const int datalink, const int expiry)
{
    ...
    if (ether_type == ETHERTYPE_IP) {
        if (pkt_len < l2_len + sizeof(ipv4_hdr_t))
                return FLOW_ENTRY_INVALID;
        ...
        if (pkt_len < l2_len + sizeof(ipv6_hdr_t))
                return FLOW_ENTRY_INVALID;
    ...
    switch (protocol) {
    case IPPROTO_UDP:
        if (pkt_len < (l2_len + ip_len + sizeof(udp_hdr_t)))
            return FLOW_ENTRY_INVALID;
        udp_hdr = (udp_hdr_t*)(packet + ip_len + l2_len);
        entry.src_port = udp_hdr->uh_sport;
        entry.dst_port = udp_hdr->uh_dport;
        break;

    case IPPROTO_TCP:
        if (pkt_len < (l2_len + ip_len + sizeof(tcp_hdr_t)))
            return FLOW_ENTRY_INVALID;
        tcp_hdr = (tcp_hdr_t*)(packet + ip_len + l2_len);
    ...
    case IPPROTO_ICMP:
    case IPPROTO_ICMPV6:
        if (pkt_len < (l2_len + ip_len + sizeof(icmpv4_hdr_t)))
            return FLOW_ENTRY_INVALID;
        icmp_hdr = (icmpv4_hdr_t*)(packet + ip_len + l2_len);
