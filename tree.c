...
*/
void tree_buildcidr(const void * treeentry, const VISIT which, const int depth, void *arg) {
  BUILDCIDR * bcdata;
  TREE * tree;
  CIDR * newcidr;
  ...
  case leaf:
    /* fall */
  case postorder:
    /* in cases of leaves and last visit add to cidrdata if necessary */
...
/* 
   uses rbwalk to check to see if a given ip address of a given type
   in the tree is inside any of the cidrdata

   since this is void, we return via the global int checkincidr
*/
void tree_checkincidr(const void * treeentry, const VISIT which, const int depth, void *arg) {
  BUILDCIDR * bcdata;
  TREE * tree;
...
int process_tree() {
  ...
  for (mymask = max_mask; mymask <= min_mask; mymask ++) {
    ...
    if (checkincidr == 0) { /* didn't find any clients in cidrdata */
      ...
      cidrdata = NULL;       /* reset to null, so when we test, all is ok */
    }
  }
...
void tree_to_cidr(const int masklen, const int type) {
...
*/
int check_ip_tree(const unsigned long ip) {
  TREE * tree;
  TREE * finder;
...
void add_tree(const unsigned long ip, const u_char * data) {
  TREE * tree;
  TREE * newtree;
...
void tree_calculate(const void * treeentry, const VISIT which, const int depth, void *arg) {
  TREE * tree;
...
*/
static int tree_comp(const void *first, const void *second, const void *config) {
  TREE * t1;
  TREE * t2;
...
TREE * packet2tree(const u_char * data) {
  TREE * mytree;
...
static void print_tree(const char * name, const TREE * tree) {
  if (tree == NULL) {
...
static void tree_nodeprint (const void *treeentry, const VISIT which, const int depth, void * arg) {
  TREE * tree;
