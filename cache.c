...
/* 
   simple function to read in a cache file created with tcpprep
   this let's us be really damn fast in picking an interface to
   send the packet out
   returns number of cache entries read

   now also checks for the cache magic and version
*/

int read_cache(char * cachefile) {
  u_int packets = 0;
  int cachefd;
  ssize_t read_size = 0;
  CACHE * cache_ptr;
...
/* 
   writes out the contents of global CACHE * cachedata to out_file
   returns the number of cache entries (bits) written (not including
   the file header (magic + version = 11 bytes)
*/
u_int write_cache(const int out_file) {
  CACHE *mycache;
  int last = 0;
  int chars;
  ssize_t written;
  ...
  while (!last) {
    ...
    /* if that was the last, stop processing, otherwise wash, rinse, repeat */
...
void add_cache(const int truefalse) {
